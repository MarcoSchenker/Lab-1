...
function IA(){this.esHumano=false;this.estrategiaDeJuego=null;}
IA.prototype.clasificar=function(carta){
if(carta.valor<=6)return 0;
else if(carta.valor<=10)return 1;
else return 2;
}
...
if(quienCanto[i]==='H')
switch(cantos[i]){
case'E':this.envidoS.push(puntos);break;
case'EE':this.revire.push(puntos);break;
case'R':this.realEnvido.push(puntos);break;
case'F':this.faltaEnvido.push(puntos);break;
}
}
return;
}
...
if(cartas[i].valor<=6)baja++;
else if(cartas[i].valor<=10)media++;
else alta++;
...
switch(ultimo){
case'T':return{now:2,next:'RT',pnext:3};
case'RT':return{now:3,next:'V',pnext:4};
case'V':return{now:4,next:'',pnext:4};
default:return{now:1,next:'T',pnext:2};
}
}
...
if(this.puntosGanadosEnvido>=3&&ran<=66)
return'N';
else
return'S';
}
...
}else if(this.gane(0)===0){
if(suMesa===null&&posiblesCartas!==null)suMesa=posiblesCartas[0];
if(miMesa!==null&&miMesa.valor<8)return'N';
if(suMesa!==null&&this.laMato(suMesa)===-1)return'N';
switch(ultimo){
case'T':
if(suMesa!==null&&this.laMato(suMesa)>-1)return'RT';
if(mediaalta>0)
return'S';
return'N';
case'RT':
if(suMesa!==null&&this.laMato(suMesa)>-1)return'V';
...
if(this.cartasEnMano[0].valor>=11)
return'S';
if(this.cartasEnMano[0].valor>=8)
return'S';
if(this.cartasEnMano[0].valor>=7){
if(this.puntosGanadosEnvido>=3)
return'N';
...
}else if(ultimo===null||ultimo===undefined){
var mato=null;
switch(nroMano){
case 0:
if(mediaalta>=3)return'T';
return'';
case 1:
if(this.gane(0)>0){
...
if(clasif.baja===1&&clasif.alta===1)
return'';
...
if(ultimaCarta.puntosEnvido>puntos)return'';
...
if(nro===0&&this.cartasEnMano[indice].valor>=11&&carta.valor<=7)
indice=this.elegir(0);
}
}else{
switch(nro){
case 0:
if(clasif.alta>=2)indice=this.elegir(0);
else if(clasif.alta>=1){
indice=this.elegir(1,null,1);
if(indice<0)indice=this.elegir(1,null,0);
}else indice=this.elegir(1);
break;
case 1:
