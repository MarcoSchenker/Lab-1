...
const SkinPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // 🔥 NUEVO: Obtener la skin actualmente seleccionada
      const selectedSkinResponse = await api.get('/api/skins/selected', axiosConfig());
      if (selectedSkinResponse.data) {
        setSelectedSkin(selectedSkinResponse.data);
        console.log('Skin seleccionada cargada:', selectedSkinResponse.data);
      }
  ...
  const handleSelectSkin = async (skinId: number) => {
    try {
      ...
      const newSelectedSkin = allSkins.find((skin) => skin.id === skinId) || null;
