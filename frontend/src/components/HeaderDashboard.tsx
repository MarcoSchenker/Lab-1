// `frontend/src/components/HeaderDashboard.tsx`
...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        showNotification({ type: 'error', text: 'No se pudieron cargar algunos datos del perfil.' });
      }
    };
  ...
  const handleUpload = async () => {
    if (!file || !loggedInUser) {
      showNotification({ type: 'error', text: 'Por favor selecciona una imagen y asegúrate de que tu sesión esté activa.' });
      return;
    }
    ...
    try {
      ...
      showNotification({ type: 'success', text: 'Foto de perfil actualizada con éxito' });
    } catch (err) {
  ...
  const tweenTransition: any = { type: 'tween', ease: 'easeOut', duration: 0.25 };
  const springTransition: any = { type: 'spring', stiffness: 300, damping: 30, mass: 0.7 };
  ...
  return (
      <div className="header-component">
        {notification && (
            <div className={`fixed top-16 right-4 z-50 p-2 rounded ${notification.type === 'success' ? 'bg-green-600 text-white' : notification.type === 'error' ? 'bg-red-600 text-white' : 'bg-blue-600 text-white'}`}>
              {notification.text}
            </div>
        )}
          ...
          <div className="profile-info">
            ...
            <div className="profile-icon-wrapper" title={isAnonymous ? "Usuario anónimo" : "Perfil"}>
              {userImage ? (
                  ...
                  <div className="profile-icon profile-icon-placeholder" onClick={toggleDropdown}>
                    <FaUser />
                  </div>
              )}
            </div>
              ...
              >
                <div className="dropdown-menu-profile">
                  <div className="profile-avatar-container">
                    {userImage ? (
                        <div className="profile-avatar" style={{ backgroundImage: `url(${userImage})` }} />
                    ) : (
                ...
                <div className="dropdown-menu-section upload-section">
                  ...
                  <div className="upload-buttons">
                    <input id="file-upload" type="file" accept="image/*" onChange={handleFileChange} style={{ display: 'none' }} ref={fileInputRef} />

                    <button className="custom-file-upload" onClick={handleUploadClick}>Seleccionar archivo</button>

                    <button className={`upload-button ${!file || userId === null ? 'disabled' : ''}`} onClick={handleUpload} disabled={!file || userId === null}>
                      Subir foto
                    </button>
                  </div>

                  {selectedFileName && <div className="file-selected">{selectedFileName}</div>}
                </div>

                <div className="actions-container">
                  <button className="profile-action-button modify-profile-button" onClick={() => navigate('/modificar-perfil')}>
                    <span>Modificar Perfil</span>
                    <IoIosArrowForward />
                  </button>

                  <button className="profile-action-button stats-button" onClick={() => userId && navigate('/user/' + userId)}>
                    <span>Ver Estadísticas Completas</span>
                    <FaChartLine />
                  </button>

                  <button className="profile-action-button delete-profile-button" onClick={() => navigate('/eliminar-perfil')}>
